...
## Installing on members' phones

There's no App Store/Play Store app here — it's the same web page, made installable as a
PWA (Progressive Web App), so it gets a real icon on the home screen and opens full-screen
without browser address bars. This is the standard way to make a site "feel like an app"
without the overhead of native app development and store review.

**Files added for this**: `manifest.json`, `sw.js` (minimal service worker), and an
`icons/` folder — copy all three into your Vercel repo alongside `checkin.html`, at the
site root (so `/manifest.json`, `/sw.js`, and `/icons/...` resolve correctly). If your repo
structure nests `checkin.html` in a subfolder, adjust the paths in that file's `<head>` and
in `manifest.json`'s `start_url` to match.

**What members actually do:**

- **Android (Chrome)**: after visiting the page once, they'll see an "Add Compass to your
  home screen" banner (built into the page) — one tap installs it. If they miss the banner,
  they can also use Chrome's menu → "Install app."
- **iPhone (Safari)**: iOS doesn't allow an automatic install prompt, so the page shows a
  banner with instructions instead: tap the **Share** icon → **Add to Home Screen**. This is
  the only way to install a web app on iOS — there's no way to trigger it programmatically,
  so a text/flyer with these exact steps is worth having at intake.

Either way, once installed it behaves like a normal app icon — opens straight to check-in,
no browser bar, and (per the trip-tracking caveats above) still only tracks location while
it's open and the screen is on.
